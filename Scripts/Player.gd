...
var _sprint_button_was_pressed = false

#--- VAR BARU UNTUK TOGGLE SPRINT ---
...
func _get_inputs():
	...
	var keyboard_sprint_just_pressed = Input.is_action_just_pressed("ui_sprint")
	var mobile_sprint_just_pressed = (sprint_button and sprint_button.is_pressed() and not _sprint_button_was_pressed)

	if keyboard_sprint_just_pressed or mobile_sprint_just_pressed:
		_sprint_toggled = not _sprint_toggled

	_is_sprint_pressed = _sprint_toggled
	...
	if sprint_button: _sprint_button_was_pressed = sprint_button.is_pressed()
...
func handle_movement_input(_delta):
	...
	if _input_dir.length() > 0:
		...
		var desired_local_rotation = target_rotation - self.global_rotation.y
		
		body.rotation.y = lerp_angle(body.rotation.y, desired_local_rotation, 0.1)
...
func handle_movement_animation():
	var forward_blend_vec = Vector2(0, -1)
	var speed_multiplier = _input_dir.length() 
	...
	if is_jumping:
		...
		if _is_sprint_pressed:
			...
			animation_tree.set("parameters/sprint/blend_position", forward_blend_vec * speed_multiplier)
		
		else:
			if animation_state.get_current_node() != "walk": 
				animation_state.travel("walk")
			
			animation_tree.set("parameters/walk/blend_position", forward_blend_vec * speed_multiplier)
